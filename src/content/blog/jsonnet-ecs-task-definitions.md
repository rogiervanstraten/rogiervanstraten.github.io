...
The task definition and the resources around it were never the same kind of change. One is "here's the cluster, the IAM role, the networking," which is infra's job and changes rarely. The other is "here's what container runs," the team's job, changing every deploy. Terraform had them zipped together, so a deploy-shaped change kept going through an infra-shaped process.
